...
for (i in 1:3){
  ...
  var.c = as.data.frame(rnorm(n = 60, 
                              mean = study$var.cont_val, 
                              sd = study$var.cont_val*0.10))
  ...
  mean_ab.sp1 = study$abun.moy * (runif(1, min=0.1, max=1.5)) # abondance moyenne de l'espece entre 10 et 150%
  ...
  mean_ab.sp3 = study$abun.moy * (runif(1, min=0.1, max=3)) # definit abondance moyenne de l'espece
  ...
  mean_ab.sp6 = study$abun.moy * (runif(1, min=0.1, max=2)) # definit abondance moyenne de l'espece
  ...
  ############################## Synthese #############################
  mat.complet = mat %>%
    arrange(mat[,2]) %>% # use the second column to sort the dataset before pasting the species
    mutate(sp1, sp2, sp3, sp4, sp5, sp6, sp7, sp8) %>%
    arrange(site)
  
  ############################# Save as CSV file ########################
  write.csv(x = mat.complet,
            file = here::here("data", "cc1", "2022", "raw.datasets", paste(row.names(study), ".csv", sep = "")))
  
} ## loop end here


### plots
boxplot (sp8~ peche, data = mat.complet, 
         main = "sp8")
plot(sp8~salinite, data = mat.complet)
